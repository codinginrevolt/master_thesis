...
pub fn get_results_mr(edens: Array2<f64>, pressures: Array2<f64>, config: &Config, rtol: &ArrayOrScalar , atol: &ArrayOrScalar) -> Result<Array3<f64>, Box<dyn Error>> {
    ...
    .for_each(|(i, mut result_slice)| 
    {
        ...
        let (masses, radii, core_pressures) = get_mr_from_eos(
            &mut e_array,
            &mut p_array,
            2.0,
            tov_iters,
        ...
        result_slice.slice_mut(s![0, ..]).assign(&masses);
        result_slice.slice_mut(s![1, ..]).assign(&radii);
...
pub fn get_results_tidal(edens: Array2<f64>, pressures: Array2<f64>, cs2s: Array2<f64>, config: &Config, rtol: &ArrayOrScalar , atol: &ArrayOrScalar) ->  Result<Array3<f64>, Box<dyn Error>> {
    let eos_num = edens.nrows();
    ...
    .for_each(|(i, mut result_slice)| 
    {
        ...
        let (masses,radii, lambdas, core_pressures) = get_mr_tidal_from_eos(
            &mut e_array,
            &mut p_array,
            &cs2_array,
            2.0,
...
pub fn get_debug_results(edens: Array2<f64>, pressures: Array2<f64>, cs2s: Array2<f64>, config: &Config, rtol: &ArrayOrScalar , atol: &ArrayOrScalar) ->  Result<Array3<f64>, Box<dyn Error>> {
    let eos_num = edens.nrows();
    ...
    for i in 0..(edens.nrows()) {
        ...
        let (masses,radii, ys, pressures) = get_mry_single_pcore(
            &mut e_array,
            &mut p_array,
            &cs2_array,
            config.settings.p_core.expect("Specify core pressure in the toml file."),
            &rtol,
            &atol
        )?;
    ...
    for (i, (masses, radii, ys, pressures)) in results_vec.into_iter().enumerate() {
        ...
        results.slice_mut(s![0, i, ..]).assign(&pad(masses));
        results.slice_mut(s![1, i, ..]).assign(&pad(radii));
