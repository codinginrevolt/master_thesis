...
def alpha_s(mu, X):
    numerator = (
        ...
        * (
            1.0
            - (64.0 * log(log(0.777632 * mu**2 * X**2)))
            / (81.0 * log(0.777632 * mu**2 * X**2))
        )
    )
    denominator = 9.0 * log(0.777632 * mu**2 * X**2)
...
def das_dmu(mu, X):
    numerator = (
        -2.20644
        - 2.79253 * log(0.777632 * mu**2 * X**2)
        + 4.41288 * log(log(0.777632 * mu**2 * X**2))
    )
...
def d2as_dmu2(mu, X):
    numerator1 = (
        22.0644
        + 2.79253 * (log(0.777632 * mu**2 * X**2)) ** 2
        - 26.4773 * log(log(0.777632 * mu**2 * X**2))
    )
    numerator2 = log(0.777632 * mu**2 * X**2) * (
        13.3765 - 4.41288 * log(log(0.777632 * mu**2 * X**2))
    )
...
def speed2_QCD(mu, X):
    ...
    dn_dmu1 = pFD(mu) * (
        d2p_das2(a_s, X) * (das_dmu(mu, X)) ** 2 + dp_das(a_s, X) * d2as_dmu2(mu, X)
    )
    dn_dmu2 = 2.0 * dp_das(a_s, X) * das_dmu(mu, X) * dpFD(mu) + p_as * d2pFD(mu)
...
def pressure_QCD(mu, X):  # MeV/fm-3
    ...
    return (PNLO(a_s) + PNNLO(a_s, X) + PN3LO(a_s)) * pFD(mu) * GeV3_to_fm3 * 1.0e3
...
class pQCD:
    ...
    def speed2(self, mu):
        ...
        dn_dmu1 = pFD(mu) * (
            d2p_das2(a_s, self.X) * (das_dmu(mu, self.X)) ** 2
            + dp_das(a_s, self.X) * d2as_dmu2(mu, self.X)
        )
        dn_dmu2 = 2.0 * dp_das(a_s, self.X) * das_dmu(mu, self.X) * dpFD(
            mu
        ) + p_as * d2pFD(mu)
    ...
    def constraints(self, e0, p0, n0, muQCD=2.6, cs2=1):
        ...
        deltaP = pQCD - p0
        pMin = cs2 / (1.0 + cs2) * (muQCD * (muQCD / mu0) ** (1.0 / cs2) - mu0) * n0
        pMax = cs2 / (1.0 + cs2) * (muQCD - mu0 * (mu0 / muQCD) ** (1.0 / cs2)) * nQCD
        nMax = nQCD * (mu0 / muQCD) ** (1.0 / cs2)
