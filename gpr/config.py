...
def parse_config(raw_config) -> SamplingConfig:
    ...
    if s["use_mu_obs"]==True:
        convert_eos = True
        return_only_eos = True
        return_connecting = True
        use_cs2 = True
    ...
    if not ((n_ceft_end == 0.0) or (0.5 <= n_ceft_end <= 2)):
        raise ValueError(
            "last number density until which to trust chiral eft must be between 0.5 and 2, or set to a hyperparameter."
        )
    ...
    if not isinstance(s["use_cs2"], bool):
        raise TypeError("use_cs2 must be a boolean")

    if not isinstance(s["use_mu_obs"], bool):
        raise TypeError("use_mu_obs must be a boolean")
