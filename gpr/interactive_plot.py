...
phi_ceft_sigma = pc.CI_to_sigma(phi_ceft_upper - phi_ceft_lower, 75)
    ...
    [
        ...
        html.Div(
            [
                dcc.Dropdown(
                    id="kernel_type",
                    options=[
                        {"label": "Squared Exponential", "value": "SE"},
                        {"label": "Matern 3/2", "value": "M32"},
                        {"label": "Matern 5/2", "value": "M52"},
                        {"label": "Gamma-Exponential", "value": "GE"},
        ...
        html.Div(
            [
                ...
                dcc.Slider(id="l", min=0.1, max=6, step=0.2, value=1),
                html.Label(
                    dcc.Markdown(
                        "**$\\gamma$** (only for Gamma exponential)", mathjax=True
                    )
                ),
                dcc.Slider(
                    id="gamma",
                ...
                dcc.Slider(
                    ...
                    step=0.1,
                    value=1,
                    marks={0.1: "0.1", 2: "2", 4: "4", 6: "6", 8: "8", 10: "10"},
                ),
            ]
        ),
...
def update_gp(kernel_type, sigma, l, gamma, alpha):
    ...
    if kernel_type == "M32":
        kernel = Kernel("M32", sigma=sigma, l=l)
    if kernel_type == "M52":
        kernel = Kernel("M52", sigma=sigma, l=l)
    if kernel_type == "GE":
    ...
    for i in range(samples.shape[0]):
        fig.add_trace(
            go.Scatter(
                ...
                showlegend=(
                    i == 0
                ),  # Show only one legend entry for all samples, or set to True for all
            )
        )
