...
class GP:
    ...
    def posterior(
        ...
        if mu is None and cov is None:
            print(
                "Use the fit function first before producing the GP or provide mean and kernel"
            )
    ...
    def _set_K_11(
        self,
        x1: np.ndarray,
        var_f: None | np.ndarray | float = None,
        stabilise: bool = False,
        jitter_value: float = 1e-10,
    ) -> np.ndarray:
    ...
    def _compute_cholesky(
        self, K_11: np.ndarray, f: np.ndarray, sampling: bool = True
    ) -> np.ndarray | tuple:
        if sampling:
            o_a = True
        else:
        ...
        try:
            ...
            L = cholesky(
                K_11, lower=True, overwrite_a=o_a, check_finite=False
            )  # algo 2.1: Line 2
        except Exception as e:
            raise ValueError(
                "The matrix K_11 probably is not semi-positive definite. Using a jitter value (set stabilise=True), or increasing it (default jitter_value=1e-10) can help with numerical stability."
            ) from e
