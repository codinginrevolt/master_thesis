...
def make_conditioning_eos():
    ...
    return n_ceft/0.16, cs2_ceft_avg, phi_ceft_sigma, e_ini, p_ini, mu_ini, n_crust, e_crust, p_crust, cs2_crust

def append_eos_results(results_e, results_p, results_cs2, epsilon, pressure, cs2, e_crust, p_crust, cs2_crust):
    e_array = np.concatenate((e_crust, epsilon))
    p_array = np.concatenate((p_crust, pressure))
    cs2_array = np.concatenate((cs2_crust, cs2))
...
def append_n_phi(results_n, results_phi, n_array, phi_array, n_crust, cs2_crust):
    n_array = np.concatenate((n_crust, n_array))
    phi_array = np.concatenate((get_phi(cs2_crust), phi_array))
